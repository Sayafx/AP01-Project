    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {                   //只能是float4或half4
                ...
                //向量点积
                float3 hDotn = dot(hDir, nDir);
                float3 BlinnPhong = pow(max(0.0, hDotn), _SpecularPow);

                return float4(BlinnPhong,1.0);         //最终输出颜色，只能是float4，float等也可以只不过会自行转换
            }
            ENDCG
        }
    }
